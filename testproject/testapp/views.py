...
def list_or_create_book(request, *args, **kwargs):
    if request.method == "GET":
        ...
        print(data)
        result, _ = Book.objects.get_or_create(title=data["title"], author=data["author"], volume=data["volume"])
        return HttpResponse(result)
...
async def async_get_book(request, *args, **kwargs):
    ...
    # print(args)
    # print(kwargs)
    result = await sync_to_async(AsyncBook.objects.get, thread_sensitive=True)(id=kwargs['pk'])
